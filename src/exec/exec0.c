...
static bool	do_cmd(t_cmd *cmd, t_env *env, int pipefd[2])
{
	cmd->pid = fork();
	if (cmd->pid == -1)
		return (exit_bad_fork(cmd), false);//quel comportement ? Fin de la commande et on continue ? Fin de la chaine de cmd ?
	else if (cmd->pid == 0)
	{
