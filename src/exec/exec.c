...
static void exec_cmd(t_cmd *cmd, char **envp, t_shell *shell) {
  if (is_builtin(cmd)) {
    ...
    exit_fork(cmd, shell); // on free tout + close fd_in et out ? puis fait un
                           // exit dedans pour kill l'enfant
...
static bool do_cmd(t_cmd *cmd, t_shell *shell, int pipefd[2]) {
  ...
  if (cmd->pid == -1)
    ...
    if (cmd->next)             // si on n'est pas a la derniere commande
      cmd->fd_out = pipefd[0]; // on ecrit dans le pipe pour envoyer a la
                               // prochaine commande
    return (true);
  }
  return (true);
...
static bool handle_single_cmd(t_cmd *cmd, t_shell *shell) {
  ...
  } else // faut stocker les STD(dup), puis dup2, puis executer, puis
         // restaurer(dup2), puis fermer
  {
  ...
  return (cmd->exit_status ==
          0); // return 1 = true si ca a fonctionne, 0 = false sinon
}
...
bool execution(t_shell *shell, t_cmd *cmd_lst) {
  ...
  while (cmd) {
    if (!set_normal_fds(cmd, pipefd))
      return (false); // car faut pas faire les autres commandes quand on a fail
                      // le pipe ? Mais comment faire pour ne pas faire les
                      // premieres commandes dont le pipe n'a pas fail ?
    if (!handle_redir_in(cmd, cmd->redirs_in,
                         shell)) // Manage Heredoc ici et impression du message
                                 // d'erreur si necessaire
      continue; // ou return (false) ?
    if (!handle_redir_out(cmd, cmd->redirs_out))
      continue; // ou return (false) ? //Si juste des redir et pas de cmd ->
                // gerer les redir (HD et creation des outfile) et ne rien faire
                // ensuite
    if (!check_cmd(cmd))
      continue;
