/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   heredoc.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: llechert <llechert@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/10 11:32:32 by llechert          #+#    #+#             */
/*   Updated: 2025/12/18 13:19:10 by llechert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

/**
 * @brief On expand si type est unquoted uniquement
 * On reutilise la structure et les fonctions du parser
 *
 * @param line
 * @param quote_type
 * @param shell
 * @return char*
 */
static char *expand_heredoc(char *line, t_sub_type quote_type, t_shell *shell)
{
	int i;
	char *res;
	char *chunk;

	if (quote_type != SUB_UNQUOTED)
		return (ft_strdup(line));
	i = 0;
	res = ft_strdup("");
	while (line[i])
	{
		res = append_until_doll(line, &i, res);
		if (!line[i])
			break;
		i++;
		chunk = expand_var(line, &i, shell); // donc faut recuperer le shell
		res = join_and_free(res, chunk);
	}
	return (res);
}

 static int	rl_sigint_hook(void)
 {
	// printf("je suis dans le hook\n");
	if (g_signal_received == 67)
	{
		// printf("Signal recevived 67\n");
		rl_replace_line("", 0);
		rl_done = 1;
	}
	return (0);
 }

/**
 * @brief Create a heredoc pipe (permet de gerer jusqu'a 1MB de char donc
 * suffisant, pas besoin de creer un doc ?!) Ecris ce qui arrive du here_doc
 * dans le pipefd[1] (cote ecriture) Attribue pipefd[0] en fd_in de la commande
 * en question S'il y a plusieurs here_doc successifs, le fd_in de la commande
 * sera celui du dernier here_doc Verifier capacite de gestion du pipe : cat
 * /proc/sys/fs/pipe-max-size
 *
 * @param cmd
 * @param redir
 * @return true
 * @return false
 */
static void heredoc_loop(int fd_out, t_cmd *cmd, t_redir *redir, t_shell *shell, int pipefd[2])
{
	char *line;
	char *exp_line;

	signal(SIGINT, heredoc_sigint_handler);
	signal(SIGQUIT, SIG_IGN);
	rl_event_hook = rl_sigint_hook;
	while (true)
	{
		line = readline(">");
		if (g_signal_received == 67)
		{
			free(line);
			close_fds_ptr(&pipefd[0], &pipefd[1]);
			close_fds_ptr(&fd_out, NULL);
			close_fds_ptr(&cmd->fd_in, &cmd->fd_out);
			clean_post_parser(shell);
			clean_shell(shell);
			exit(130);
		}
		if (!line)
		{
			ft_putendl_fd("minishell: warning: here-document delimited by end-of-file", 2);
			break;
		}
		if (ft_strcmp(line, redir->file) == 0)
		{
			free(line);
			break;
		}
		exp_line = expand_heredoc(line, redir->file_quote_type, shell);
		free(line);
		if (!exp_line)
		{
			free(line);
			close_fds_ptr(&pipefd[0], &pipefd[1]);
			close_fds_ptr(&fd_out, &cmd->fd_in);
			clean_post_parser(shell);
			clean_shell(shell);
			exit(EXIT_FAILURE);
		}
		ft_putendl_fd(exp_line, fd_out);
		free(exp_line);
	}
	close_fds_ptr(&fd_out, &cmd->fd_in);
	clean_post_parser(shell);
	clean_shell(shell);
	exit(EXIT_SUCCESS);
}

#define HD_FILE ".heredoc_tmp"
bool create_heredoc(t_cmd *cmd, t_redir *redir, t_shell *shell, int pipefd[2])
{
	int     fd;
	pid_t   pid;
	int     status;

	// 1. On ouvre le fichier en écriture pour l'enfant
	fd = open(HD_FILE, O_CREAT | O_WRONLY | O_TRUNC, 0644);
	if (fd == -1)
		return (print_error(HD_FILE, errno, ERR_REDIR, cmd), false);
	
	pid = fork();
	if (pid == -1)
		return (close(fd), print_error(NULL, errno, ERR_FORK, cmd), false);
	
	if (pid == 0)
		heredoc_loop(fd, cmd, redir, shell, pipefd);

	// PARENT
	close(fd); // On ferme l'accès en écriture
	signal(SIGINT, SIG_IGN);
	waitpid(pid, &status, 0);
	init_signals();

	if (WIFEXITED(status) && WEXITSTATUS(status) == 130)
	{
		unlink(HD_FILE); // On nettoie si Ctrl+C
		return (cmd->exit_status = 130, false);
	}

	// 2. On ouvre le fichier en lecture pour la commande
	cmd->fd_in = open(HD_FILE, O_RDONLY);
	
	// 3. IMPORTANT : On unlink immédiatement
	// Le fichier est marqué pour suppression, mais reste accessible tant que 
	// cmd->fd_in est ouvert. Il disparaîtra tout seul à la fin.
	unlink(HD_FILE);

	if (cmd->fd_in == -1)
		return (print_error(HD_FILE, errno, ERR_REDIR, cmd), false);
	return (true);
}
